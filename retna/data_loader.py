...
class Data_Handler(Dataset):
    ...
    def randomize_data(self, im_cat ):
        ...
        if self.randomize:
            ...
            if self.noise is True: im_cat = gaussian_noise(im_cat, self.rand_noise, self.noise_range,self.noise_low,self.noise_high,self.noise_size)
        elif self.mosaic:
            scale = np.random.uniform(self.scale_range[0], self.scale_range[1])
            out_sz = np.array(self.outsize)
            crop_sz = (out_sz/scale).astype(np.int)
...
class Image_Handler(Data_Handler):
    ...
    def __init__(self, files):
        ...
        if '.jpg' in files[0]:
            inpt_fns = [fn for fn in files if fn.find(".jpg") != -1]
            targ_fns = [fn for fn in files if fn.find("_label.tif") != -1]
        
        elif '.png' in files[0]:
            inpt_fns = [fn for fn in files if fn.find(".png") != -1]
            targ_fns = [fn for fn in files if fn.find("_label.tif") != -1]
...
def expand_label(label_in, n_chans=None ):
    ...
    if n_chans is None:
        ...
        if n_chans > 100:
            label_in[label_in>0] = label_in[label_in>0] - (min(label_in[label_in>0])-1)
            n_chans = label_in.max()
        elif n_chans == 0:
            label_in = label_in[None, :]
            return label_in
...
def gaussian_noise(im_cat,prob,range, lw,hg,s):
    rand_var = np.random.rand(2)
    if rand_var[0]< prob:
        im = im_cat[0]
        a = np.random.uniform(low=0.8, high = 1.2)
        b = random_ramp(im.shape)
        b1 = np.random.uniform(low=-0.3, high=0.3)
        noise = np.random.uniform(range[0],range[1], im.shape)*np.random.uniform(0,0.5)
        im = np.clip((a*im+b1+b+noise),0,1)
        im_cat[0] = im
    return im_cat

def random_ramp(shape):
    a,b,c,d = np.random.uniform(low=-1, high = 1, size =4)
    x = np.linspace(a,b, shape[0])
    y = np.linspace(c,d, shape[1])
